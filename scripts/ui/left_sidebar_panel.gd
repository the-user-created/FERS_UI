...
func _ready() -> void:
	# Connect to the global data store instead of relying on a parent.
...
func _on_simulation_data_element_selected(element_id: String) -> void:
	...
	display_properties(element_id, element_data)


func _on_simulation_data_element_updated(element_id: String, element_data: Dictionary) -> void:
	# If the element that was updated is the one we are currently viewing, refresh the view.
...
func display_properties(element_id: String, element_data: Dictionary) -> void:
	if element_data.is_empty():
		clear_panel()
		_add_label("No data for selected element.")
		return
	...
	if element_type != "global_simulation_name" and element_type != "global_simulation_parameters":
		_add_string_editor("Name", "name", element_data.get("name", "Unnamed Element"), element_id)
	...
	match element_type:
		...
		"global_simulation_parameters":
			...
			_add_float_editor("Sampling Rate (Hz)", "sampling_rate", float(element_data.get("sampling_rate", 1_000_000.0)), element_id, 1.0)
		"platform":
...
func _add_string_editor(label_text: String, property_key: String, current_val: String, el_id: String) -> LineEdit:
	...
	line_edit.text_changed.connect(_on_text_property_changed.bind(el_id, property_key))
	hbox.add_child(line_edit)
	vbox_content.add_child(hbox)
	return line_edit


func _add_float_editor(label_text: String, property_key: String, current_val: float, el_id: String, min_v: float = -INF, max_v: float = INF, stp: float = 0.01) -> SpinBox:
	var hbox = HBoxContainer.new()
	var label = Label.new(); label.text = label_text + ":"; label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	hbox.add_child(label)
	var spin_box = SpinBox.new(); spin_box.min_value = min_v; spin_box.max_value = max_v; spin_box.step = stp; spin_box.value = current_val; spin_box.size_flags_horizontal = Control.SIZE_EXPAND_FILL; spin_box.allow_greater = true; spin_box.allow_lesser = true
	spin_box.value_changed.connect(_on_numerical_property_changed.bind(el_id, property_key))
...
func _add_option_button(label_text: String, property_key: String, options_arr: Array, current_selection_val: Variant, el_id: String) -> OptionButton:
	...
	option_button.item_selected.connect(_on_option_button_item_selected.bind(el_id, property_key, option_button))
...
func _add_checkbox(label_text: String, property_key: String, is_chk: bool, el_id: String) -> CheckBox:
	var check_box = CheckBox.new(); check_box.text = label_text; check_box.button_pressed = is_chk
	check_box.toggled.connect(_on_checkbox_toggled.bind(el_id, property_key))
...
func _add_file_picker(label_text: String, property_key: String, current_pth: String, el_id: String, dialog_title_str: String = "Select File") -> HBoxContainer:
	...
	var label = Label.new(); label.text = label_text + ":"; label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	hbox.add_child(label)
	var line_edit = LineEdit.new(); line_edit.text = current_pth; line_edit.editable = false; line_edit.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	hbox.add_child(line_edit)
	var button = Button.new(); button.text = "Browse..."
	var context: Dictionary = {"element_id": el_id, "property_key": property_key, "line_edit_node": line_edit, "dialog_title": dialog_title_str}
	button.pressed.connect(_on_browse_file_button_pressed.bind(context))
...
func _populate_platform_properties(el_id: String, data: Dictionary) -> void:
	var platform_types_options: Array[Variant] = [{"name": "Monostatic", "value": "monostatic"}, {"name": "Transmitter", "value": "transmitter"}, {"name": "Receiver", "value": "receiver"}, {"name": "Target", "value": "target"}]
	...
	manage_motion_button.pressed.connect(_on_manage_waypoints_pressed.bind("motion_path", "motion", data))
	...
	match rotation_model.get("type", "fixed"):
		"fixed":
			...
			_add_float_editor("Start Azimuth (deg)", "start_azimuth", float(fixed_data.get("start_azimuth", 0.0)), el_id).value_changed.connect(_on_fixed_rotation_property_changed.bind(el_id, "start_azimuth"))
			_add_float_editor("Start Elevation (deg)", "start_elevation", float(fixed_data.get("start_elevation", 0.0)), el_id).value_changed.connect(_on_fixed_rotation_property_changed.bind(el_id, "start_elevation"))
			_add_float_editor("Azimuth Rate (deg/s)", "azimuth_rate", float(fixed_data.get("azimuth_rate", 0.0)), el_id).value_changed.connect(_on_fixed_rotation_property_changed.bind(el_id, "azimuth_rate"))
			_add_float_editor("Elevation Rate (deg/s)", "elevation_rate", float(fixed_data.get("elevation_rate", 0.0)), el_id).value_changed.connect(_on_fixed_rotation_property_changed.bind(el_id, "elevation_rate"))
		"path":
			...
			manage_rot_button.pressed.connect(_on_manage_waypoints_pressed.bind("rotation_model", "rotation", data))
			vbox_content.add_child(manage_rot_button)
	vbox_content.add_child(HSeparator.new())
	var type_specific_label: Label = _add_label("Type-Specific Properties:"); type_specific_label.add_theme_font_size_override("font_size", 18)
...
func _populate_target_subtype_props(el_id: String, data: Dictionary) -> void:
	...
	if data.get("target_rcs_type_actual", "isotropic") == "isotropic":
		...
		_add_file_picker("RCS Filename", "target_rcs_filename", data.get("target_rcs_filename", ""), el_id, "Select RCS File (*.csv, *.xml)")
...
func _get_available_elements_by_type(target_element_type: String) -> Array[Dictionary]:
	return SimData.get_elements_by_type(target_element_type)


# --- Signal Callbacks for Property Changes ---
# A central function to notify the data store of a change.
func _emit_property_change(element_id: String, property_key: String, new_value: Variant) -> void:
	SimData.update_element_property(element_id, property_key, new_value)


func _on_text_property_changed(new_text: String, el_id: String, prop_key: String) -> void:
	_emit_property_change(el_id, prop_key, new_text)


func _on_numerical_property_changed(new_value: float, el_id: String, prop_key: String) -> void:
	_emit_property_change(el_id, prop_key, new_value)


func _on_checkbox_toggled(button_pressed_state: bool, el_id: String, prop_key: String) -> void:
	_emit_property_change(el_id, prop_key, button_pressed_state)
...
func _on_option_button_item_selected(item_idx: int, el_id: String, prop_key: String, option_button_node: OptionButton) -> void:
	if item_idx >= 0 and item_idx < option_button_node.item_count:
		...
		_emit_property_change(el_id, prop_key, selected_value)
	else:
		printerr("LeftSidebarPanel: Invalid index '", item_idx, "' for OptionButton '", option_button_node.name, "'")
...
func _on_file_dialog_file_selected(path: String) -> void:
	if _file_picker_context.is_empty():
		...
		return
	var line_edit_node: LineEdit = _file_picker_context.get("line_edit_node")
	if is_instance_valid(line_edit_node):
		line_edit_node.text = path
	_emit_property_change(_file_picker_context.element_id, _file_picker_context.property_key, path)
	_file_picker_context.clear()


# Special handlers for nested data structures
func _on_fixed_rotation_property_changed(new_value: float, el_id: String, prop_key: String) -> void:
	# Get a fresh copy of the data, modify it, and send the whole dictionary back.
	var element_data := SimData.get_element_data(el_id)
	var rotation_model: Dictionary = element_data.get("rotation_model")
	if rotation_model is Dictionary and rotation_model.has("fixed_rotation_data"):
		rotation_model.fixed_rotation_data[prop_key] = new_value
		_emit_property_change(el_id, "rotation_model", rotation_model)


func _on_manage_waypoints_pressed(property_key: String, type: String, element_data: Dictionary) -> void:
	_waypoint_editor_context_key = property_key
	var waypoints: Array = []
	if property_key == "motion_path":
		waypoints = element_data.get(property_key, {}).get("waypoints", [])
	elif property_key == "rotation_model":
		waypoints = element_data.get(property_key, {}).get("rotation_path_data", {}).get("waypoints", [])
...
func _on_waypoint_editor_updated(new_waypoints_array: Array) -> void:
	...
	# Get a fresh copy of the data, modify it, and send the whole dictionary back.
	var element_data := SimData.get_element_data(current_element_id)
	var data_to_update: Dictionary = element_data.get(_waypoint_editor_context_key, {})
	...
	if _waypoint_editor_context_key == "motion_path":
		...
		if not data_to_update.has("rotation_path_data"):
			data_to_update["rotation_path_data"] = {}
		data_to_update.rotation_path_data["waypoints"] = new_waypoints_array
	else:
		return

	_emit_property_change(current_element_id, _waypoint_editor_context_key, data_to_update)
